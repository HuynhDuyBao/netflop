...
async function getMovie(req, res, next) {
  try {
    const movie = await movieService.getMovieById(Number(req.params.id), req.user || null);
...
async function recordView(req, res, next) {
  try {
    const views = await movieService.recordView(Number(req.params.id));

    res.json({
      success: true,
      message: 'Da cap nhat luot xem.',
      data: { views }
    });
  } catch (error) {
    next(error);
...
async function deleteComment(req, res, next) {
  try {
    await movieService.deleteComment({
      user: req.user,
      movieId: Number(req.params.id),
